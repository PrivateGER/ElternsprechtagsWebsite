    ...
    public function getMessagesAsJSON(Request $request) {
        ...
        foreach ($messages as $message) {
            ...
            if($message["author"] === Auth::id()) {
                ...
                array_push($formattedMessages, $request->input()["name"] . ": " . $message["message"] . "|||" . $dt->format("H:i\n"));
            }
        }
        header('Content-Type: application/json');

        $stmt = $pdo->prepare("UPDATE chat_messages SET readMessage = 1 WHERE (`author` = ? AND `recipient` = ?) ORDER BY created_at");
        $stmt->bindParam(1, $recID);
        $stmt->bindParam(2, $self);
        $stmt->execute();
